 ...
 * `.value` projections in the generated Swift to use them too.
 *
...
const bridgeHeader = fs.readFileSync(
  path.join(__dirname, '..', 'nitrogen/generated/ios/MunimBluetooth-Swift-Cxx-Bridge.hpp'),
  'utf8'
)
...
function checked(bridgeType, expression) {
  for (const helper of [`has_value_${bridgeType}`, `get_${bridgeType}`]) {
    if (!bridgeHeader.includes(` ${helper}(`)) {
      throw new Error(`patch-nitro-optionals: bridge helper ${helper} not found`)
    }
  }
...
let patched = 0
for (const file of fs.readdirSync(swiftDir).filter((name) => name.endsWith('.swift'))) {
  ...
  source = source
    ...
    .map((chunk) => {
      ...
      for (const [, name, bridgeType] of signature[1].matchAll(
        ...
        result = result.replace(new RegExp(`: ${name}\\.value([,)])`, 'g'), (_, tail) => {
          patched++
...
const leftover = fs
  ...
  .flatMap((name) =>
    fs
      ...
      .split('\n')
      .filter((line) => /\b\w+\.value\b(?!\()/.test(line) && !/__result\.value|\.value\s*=/.test(line))
      .map((line) => `${name}: ${line.trim()}`)
  )
if (leftover.length > 0) {
  throw new Error(`patch-nitro-optionals: unchecked .value projections remain:\n${leftover.join('\n')}`)
}
