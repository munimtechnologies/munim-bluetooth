///
/// ManufacturerDataEntry.kt
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

package com.margelo.nitro.munimbluetooth

import androidx.annotation.Keep
import com.facebook.proguard.annotations.DoNotStrip
import java.util.Objects


/**
 * Represents the JavaScript object/struct "ManufacturerDataEntry".
 */
@DoNotStrip
@Keep
data class ManufacturerDataEntry(
  @DoNotStrip
  @Keep
  val companyId: Double,
  @DoNotStrip
  @Keep
  val data: String
) {
  /* primary constructor */

  override fun equals(other: Any?): Boolean {
    if (this === other) return true
    if (other !is ManufacturerDataEntry) return false
    return Objects.deepEquals(this.companyId, other.companyId)
      && Objects.deepEquals(this.data, other.data)
  }

  override fun hashCode(): Int {
    return arrayOf(
      companyId,
      data
    ).contentDeepHashCode()
  }

  companion object {
    /**
     * Constructor called from C++
     */
    @DoNotStrip
    @Keep
    @Suppress("unused")
    @JvmStatic
    private fun fromCpp(companyId: Double, data: String): ManufacturerDataEntry {
      return ManufacturerDataEntry(companyId, data)
    }
  }
}
